---
title: "Regresi Poisson - Harvest Walukow (164231104)"
...
## a. Fungsi Estimasi Parameter Regresi Poisson dengan Metode BHHH

```{r}
if(!require(maxLik)) {                           # install package jika belum ada
  install.packages("maxLik")
  library(maxLik)
}

poisson_loglik <- function(beta, y, X) {
  mu <- exp(X %*% beta)                          # hitung μ = exp(Xβ)
  loglik <- sum(y * log(mu) - mu - lfactorial(y)) # ln L(β) = Σ[yi*ln(μi) - μi - ln(yi!)]
  return(loglik)                                 # return positive untuk maksimisasi
}
...
```{r}
cat("Model berhasil diestimasi dengan metode BHHH")  # konfirmasi keberhasilan
cat("\nVariabel signifikan:", 
    rownames(hasil_manual)[hasil_manual$p_value < 0.05])  # variabel signifikan
``` 