...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 17 Bulan Mei 2025 pada 14.31
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `tambah_stok_produk` (IN `productCode` VARCHAR(16), IN `quantity` INT)   BEGIN
    UPDATE products
...
--
-- Stand-in struktur untuk tampilan `city_amount_max`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `city_amount_max` (
`city` varchar(50)
,`MAX(amount)` decimal(10,2)
);
...
--
-- Stand-in struktur untuk tampilan `customer_amount_terbanyak`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `customer_amount_terbanyak` (
...
--
-- Stand-in struktur untuk tampilan `nama_customer_amount_terkecil`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `nama_customer_amount_terkecil` (
`customerName` varchar(50)
);
...
--
-- Stand-in struktur untuk tampilan `total_customer_employee`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `total_customer_employee` (
`firstName` varchar(50)
,`lastName` varchar(50)
,`COUNT(customerNumber)` bigint(21)
);

-- --------------------------------------------------------

--
-- Struktur untuk view `city_amount_max`
--
DROP TABLE IF EXISTS `city_amount_max`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `city_amount_max`  AS SELECT `customers`.`city` AS `city`, max(`payments`.`amount`) AS `MAX(amount)` FROM (`customers` join `payments` on(`customers`.`customerNumber` = `payments`.`customerNumber`)) GROUP BY `customers`.`city` ;

-- --------------------------------------------------------

--
-- Struktur untuk view `customer_amount_terbanyak`
--
DROP TABLE IF EXISTS `customer_amount_terbanyak`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_amount_terbanyak`  AS SELECT `customers`.`customerNumber` AS `customerNumber`, `customers`.`customerName` AS `customerName`, `customers`.`contactLastName` AS `contactLastName`, `customers`.`contactFirstName` AS `contactFirstName`, `customers`.`phone` AS `phone`, `customers`.`addressLine1` AS `addressLine1`, `customers`.`addressLine2` AS `addressLine2`, `customers`.`city` AS `city`, `customers`.`state` AS `state`, `customers`.`postalCode` AS `postalCode`, `customers`.`country` AS `country`, `customers`.`salesRepEmployeeNumber` AS `salesRepEmployeeNumber`, `customers`.`creditLimit` AS `creditLimit` FROM `customers` WHERE `customers`.`customerNumber` = (select `payments`.`customerNumber` from `payments` group by `payments`.`customerNumber` order by sum(`payments`.`amount`) desc limit 1) ;

-- --------------------------------------------------------

--
-- Struktur untuk view `nama_customer_amount_terkecil`
--
DROP TABLE IF EXISTS `nama_customer_amount_terkecil`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `nama_customer_amount_terkecil`  AS SELECT `customers`.`customerName` AS `customerName` FROM `customers` WHERE `customers`.`customerNumber` = (select `payments`.`customerNumber` from `payments` where `payments`.`amount` = (select min(`payments`.`amount`) from `payments`)) ;

-- --------------------------------------------------------

--
-- Struktur untuk view `total_customer_employee`
--
DROP TABLE IF EXISTS `total_customer_employee`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `total_customer_employee`  AS SELECT `employees`.`firstName` AS `firstName`, `employees`.`lastName` AS `lastName`, count(`customers`.`customerNumber`) AS `COUNT(customerNumber)` FROM (`customers` join `employees` on(`customers`.`salesRepEmployeeNumber` = `employees`.`employeeNumber`)) GROUP BY `employees`.`employeeNumber` ;
